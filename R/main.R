...
scalegram <- function(x, MODE = "s1", STD = TRUE, threshold = 30){ #STD: standardize the scalegram, threshold: sample size at last scale
  ...
  if (MODE %!in% c("mu", "s1", "s2", "s3", "s4", "CV",
    ...
    if (max_agg_scale != 0 & nna > 2 * threshold){ # check for adequate time series length
      ...
      if (STD == TRUE){ # standardize data ,i.e., zero mean unit variance at the original time scale
        samp <- scale(x, center = TRUE, scale = TRUE)
      }
      ...
      if (MODE == "mu"){ # MODE: mean
        ...
        scale_df[1,"y_scale"] <- Lmoments(x, rmax=2, na.rm=T)[, "L1"]
      ...
      for (i in 2:max_agg_scale){ # loop around aggregation scale
        ...
        if (sum(!is.na(x_agg))>=threshold){ # have at least 30 values for the y_scale estimation     # normal moments -------------------------------------------------------
          if (MODE == "mu"){ # MODE: mean
            ...
            scale_df[i,"y_scale"] <- Lmoments(x_agg, rmax=2,na.rm=T)[,"L1"]
